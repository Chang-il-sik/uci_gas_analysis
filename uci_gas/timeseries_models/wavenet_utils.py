...
def dilate(x, dilation, init_dilation=1, pad_start=True):
    ...
    [n, c, l] = x.size()
    dilation_factor = float(dilation) / init_dilation
    if dilation_factor == 1:
        return x

    # zero padding for reshaping
    try:
        new_l = int(np.ceil(float(l) / dilation_factor) * dilation_factor)
    except:
        print(dilation_factor)
    if new_l != l:
        l = new_l
        # print('[1] dilate x : ', x.shape)
        x = constant_pad_1d(x, new_l, dimension=2, pad_start=pad_start)
        # print('[2] dilate x : ', x.shape)
    ...
    # print('[3] dilate x : ', x.shape)
    x = x.permute(1, 2, 0).contiguous()  # (n, c, l) -> (c, l, n)
    # print('[4] dilate x : ', x.shape)
    x = x.view(c, l, n)
    # print('[5] dilate x : ', x.shape)
    x = x.permute(2, 0, 1).contiguous()  # (c, l, n) -> (n, c, l)
    # print('[6] dilate x : ', x.shape)
