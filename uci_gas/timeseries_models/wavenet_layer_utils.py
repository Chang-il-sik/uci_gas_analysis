...
from .wavenet_utils import dilate
...
class ResidualBlock(nn.Module):
	...
	def forward(self,inputs,dilation,init_dilation):
		# print('[1]ResidualBlock st dilate :', inputs.shape, dilation, init_dilation)
		inputs = dilate(inputs,dilation,init_dilation)
		# print('[1]ResidualBlock ed dilate :', inputs.shape)
		sigmoid_out = self.gate_conv(inputs)
		sigmoid_out = self.gate_norm(sigmoid_out)
		sigmoid_out = torch.sigmoid(sigmoid_out)        
		...
		skip = hidden
		# print('ResidualBlock hidden :', hidden.shape, hidden.size(2))
		if hidden.size(2)!=1:
			# print('[2]ResidualBlock st dilate :', hidden.shape, 1, dilation)
			skip = dilate(hidden,1,init_dilation=dilation)
			# print('[2]ResidualBlock ed dilate :', skip.shape)
